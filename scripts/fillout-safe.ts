...
function extractSlackId(slackInput: string): string {
	const patterns = [/\b(U[A-Z0-9]{8,})\b/, /\(([^)]+)\)/, /\/\s*(\S+)$/];
...
function mergeSubmissions(submissions: FilloutSubmission[]): any {
	...
	const { firstName, lastName } = extractFirstLastName(
		latestSubmission['To confirm, please e-sign below.']
	);
	if (firstName) merged['First Name'] = firstName;
	if (lastName) merged['Last Name'] = lastName;

	// Address fields
	if (latestSubmission['Address (Your address)'])
		merged['Address (Line 1)'] = latestSubmission['Address (Your address)'];
	if (latestSubmission['City (Your address)'])
		merged['City'] = latestSubmission['City (Your address)'];
	if (latestSubmission['State/Province (Your address)'])
		merged['State / Province'] = latestSubmission['State/Province (Your address)'];
	if (latestSubmission['Zip/Postal code (Your address)'])
		merged['ZIP / Postal Code'] = latestSubmission['Zip/Postal code (Your address)'];
	if (latestSubmission['Country (Your address)'])
		merged['Country'] = latestSubmission['Country (Your address)'];

	// Feedback
	merged['How did you hear about this?'] =
		latestSubmission['Where did you find Converge?'] ||
		latestSubmission["How'd you hear about Converge?"];
	...
	for (const submission of submissions) {
		...
		// Project 2 (if exists)
		const hasSecondProject =
			submission['I made a second project'] === 'true' ||
			submission['I made a second project'] === 'Yes';
		if (hasSecondProject) {
