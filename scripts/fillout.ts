...
function mapFilloutToAirtable(submission: FilloutSubmission, projectIndex: number = 1): any {
	...
	const { firstName, lastName } = extractFirstLastName(
		submission['To confirm, please e-sign below.']
	);
	...
	const fields: any = {
		...
		'How did you hear about this?':
			submission['Where did you find Converge?'] || submission["How'd you hear about Converge?"],
...
async function syncFilloutToAirtable() {
	...
	for (const submission of submissions) {
		...
		if (matchingRecords.length === 0) {
			console.log(
				`No pending record found for ${email || slackId} - checking if they need new records created...`
			);

			// Check if this submission has any projects
			const hasFirstProject =
				submission['Project repo link (1)'] ||
				submission['Link to deployment'] ||
				submission['Hackatime project names'];
			if (!hasFirstProject) {
		...
		if (projectCount > matchingRecords.length) {
			...
			console.log(
				`  ⚠️  ${email} has ${projectCount} projects but only ${matchingRecords.length} pending records`
			);
