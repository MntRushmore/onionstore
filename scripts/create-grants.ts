...
async function createHCBGrant(grantRequest: HCBGrantRequest): Promise<HCBGrantResponse | null> {
	try {
		...
		if (!response.ok) {
			...
			console.error(`Failed to create grant for ${grantRequest.email}: ${response.status} ${response.statusText}`);
			console.error(`Error details: ${errorText}`);
			return null;
		}

		const result: HCBGrantResponse = await response.json();
		console.log(`✓ Created grant for ${grantRequest.email}: $${(grantRequest.amount_cents / 100).toFixed(2)} (ID: ${result.id})`);
		return result;
	} catch (error) {
	...
	try {
		...
		if (LOOPS_API_KEY) {
			...
			if (response.ok) {
				...
				console.error(`✗ Failed to send fulfillment email to ${allocation.email}: ${await response.text()}`);
			}
		} else {
...
async function processGrantFile(filename: string): Promise<void> {
	...
	try {
		...
		const results: Array<{ request: HCBGrantRequest; response: HCBGrantResponse | null; allocation: GrantAllocation }> = [];
		...
		const totalOrdersFulfilled = successfulAllocations.reduce((sum, allocation) => sum + allocation.orderIds.length, 0);
		...
		if (failed.length > 0) {
			...
			for (const failedGrant of failed) {
				console.log(`- ${failedGrant.email}: $${(failedGrant.amount_cents / 100).toFixed(2)} for ${failedGrant.purpose || 'N/A'}`);
			}
		}
...
async function main() {
	...
	if (!filename) {
		...
		console.error('Example: bun run scripts/create-grants.ts grant-requests-2025-08-29T10-30-00-000Z.json');
		process.exit(1);
	}
