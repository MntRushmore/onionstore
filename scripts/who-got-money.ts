...
function calculateHoursFromProjects(hackatimeData: HackatimeResponse, targetProjectNames: string[]): number {
...
async function processConvergeSubmissions() {
  ...
  const warnings: Array<{ slackId: string; email: string; oldBalance: number; newBalance: number; difference: number }> = [];
